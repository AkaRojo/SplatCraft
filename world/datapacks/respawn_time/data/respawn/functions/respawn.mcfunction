...
execute as @a[tag=respawn.respawning,limit=1,sort=nearest] if entity @a[tag=respawn.respawning,limit=1,sort=nearest] store result entity @e[type=armor_stand,tag=respawn.point,limit=1] Rotation[0] float 1 run data get entity @s SpawnAngle 1

execute rotated as @e[type=armor_stand,tag=respawn.point,limit=1] run teleport @a[tag=respawn.respawning,limit=1,sort=nearest] @e[type=armor_stand,tag=respawn.point,limit=1]
